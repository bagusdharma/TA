    ...
    for cand in bottom_cand:
        ...
        for r in bottom_cand:
            if r is cand:
                ...
                # kalau dia mendominasi, berarti bukan yang dibawah
                # atau node nya ada diatas (bukan dibawah/terbawah)
...
def generick_target(attr_input, attribute):
    target = basic_target()
    # nilai atribut nya waktu di input dipisah dengan ','
    attr_value = attr_input.split(',')
    # for untuk banyaknya atribut yang dimasukkan , misal 500,400,500 (3 atribut)
    for x in range(len(attribute)):
        # nilai input tadi menjadi nilai dari atribut target dan dijadikan nilai integer
...
def calculate_cost(preference, g, attribute):
    ...
    is_first = True
    for attr in attribute:
        # hitung nilai cost dari tiap preferensi
...
def search_clean_cut(graph, node, target, edges, attribute, is_initial = False):
    cut = list()
    if not is_initial:
        # kalau si node sudah dikunjungi tulis seperti ini dan bukan sebuah initial / v root
        if node in visited:
    ...
    if is_initial or is_dominating(graph[node], target, attribute):
        if is_initial:
            # ketika dia adalah root, maka child itu diberikan key
            child_dict = {k: v for k, v in graph.iteritems() if v["is_root"]}
            child = child_dict.keys()
        else:
            # sisanya di apend ke list cut
            cut.append(node)
            # dengan isi childnya 'to' node di garis tujuan dan 'from' node asal
